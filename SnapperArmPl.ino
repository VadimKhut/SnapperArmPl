...
void setup(){
	...
	doArmIK(true, g_sIKX,g_sIKY,g_sIKZ,g_sIKGA);					// X=0; Y=150; Z=150; GA=0 
	MoveArmTo(sBase, sShoulder, sElbow, sWrist, sWristRot, sGrip);

	SetServo(sDeltaTime);	//= 3000

	// start serial
	Serial.begin(9600);
	Serial.println("Starting Playback Demo");
	Serial.println("Send '1' to start or or press Button 'playback'");
...
* enable - setting this to '1' makes the function work. Setting it to '0' bypasses the function. 
*          This can be usefull for breaking out of sequences
*
*
**************************************************************************************/
